...
|---|---|---|---|
| **2.0.x** | `.NET 8.0`, `.NET 9.0`, `.NET 10.0` | ✅ Yes | Current Active Release |
...
- **Sigstore Build Provenance Attestation**: All released packages are attested with cryptographic build provenance using GitHub Actions and Sigstore (`actions/attest-build-provenance`).
- **NuGet Trusted Publishing (OIDC)**: Packages are published using OpenID Connect (OIDC) authentication (`NuGet/login`), completely eliminating static, long-lived API tokens.
