...
/// <summary>
/// Classifies MySQL error codes, deadlocks, lock wait timeouts, and unique constraint violations.
/// </summary>
public static class MySqlConcurrencyErrorClassifier
{
    ...
    /// <summary>
    /// Determines whether the specified exception represents a MySQL deadlock (Error 1213).
    /// </summary>
    ...
    /// <summary>
    /// Determines whether the specified exception represents a MySQL lock wait timeout (Error 1205).
    /// </summary>
    /// <param name="exception">The exception to inspect.</param>
    /// <returns><see langword="true"/> if the error represents a lock wait timeout; otherwise, <see langword="false"/>.</returns>
    ...
    /// </summary>
    /// <param name="exception">The MySQL exception to translate.</param>
        ...
        string? operation = null)
    {
        ...
        if (IsDeadlock(exception))
        {
            return new ConcurrencyConflict(
                ...
                message: $"MySQL deadlock detected (Error {exception.Number}): {exception.Message}",
        ...
        if (IsLockTimeout(exception))
        {
            return new ConcurrencyConflict(
                ...
                classification: ConcurrencyConflictClassification.Transient,
                operation: operation ?? "Update",
                message: $"MySQL lock wait timeout exceeded (Error {exception.Number}): {exception.Message}",
                metadata: new Dictionary<string, string>
                {
                    ["provider"] = "MySql",
                    ["errorNumber"] = exception.Number.ToString(System.Globalization.CultureInfo.InvariantCulture)
        ...
        if (IsUniqueViolation(exception))
        {
            return new ConcurrencyConflict(
                ...
                operation: operation ?? "Update",
                message: $"MySQL duplicate entry constraint violation (Error {exception.Number}): {exception.Message}",
                metadata: new Dictionary<string, string>
                {
                    ["provider"] = "MySql",
                    ["errorNumber"] = exception.Number.ToString(System.Globalization.CultureInfo.InvariantCulture)
