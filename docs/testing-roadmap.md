...
This document serves as the **single source of truth, execution guide, reproducible evidence, and idempotent tracking mechanism** for the testing, cleanup, and mutation testing strategy of `EricksonLopez.Concurrency`.
...
```
src/
├── EricksonLopez.Concurrency                    # Package component
├── EricksonLopez.Concurrency.Abstractions       # Package component
├── EricksonLopez.Concurrency.AspNetCore         # Package component
├── EricksonLopez.Concurrency.Dapper             # Package component
├── EricksonLopez.Concurrency.MariaDb            # Package component
├── EricksonLopez.Concurrency.Mediator           # Package component
├── EricksonLopez.Concurrency.MySql              # Package component
├── EricksonLopez.Concurrency.Oracle             # Package component
├── EricksonLopez.Concurrency.PostgreSql         # Package component
├── EricksonLopez.Concurrency.Result             # Package component
├── EricksonLopez.Concurrency.Sqlite             # Package component
├── EricksonLopez.Concurrency.SqlServer          # Package component
├── EricksonLopez.Concurrency.Testing            # Package component
```
...
| **U05** | `MariaDb` | `COMPONENT` | `DONE` | 100% | 100% | 100% | 100% |
| **U06** | `Mediator` | `COMPONENT` | `DONE` | 100% | 100% | 100% | 100% |
| **U07** | `MySql` | `ADAPTER` | `DONE` | 100% | 100% | 100% | 100% |
| **U08** | `Oracle` | `COMPONENT` | `DONE` | 100% | 100% | 100% | 100% |
| **U09** | `PostgreSql` | `ADAPTER` | `DONE` | 100% | 100% | 100% | 100% |
| **U10** | `Result` | `COMPONENT` | `DONE` | 100% | 100% | 100% | 100% |
| **U11** | `Sqlite` | `ADAPTER` | `DONE` | 100% | 100% | 100% | 100% |
| **U12** | `SqlServer` | `ADAPTER` | `DONE` | 100% | 100% | 100% | 100% |
| **U13** | `Testing` | `COMPONENT` | `DONE` | 100% | 100% | 100% | 100% |

---

## 4. Execution Cycle per Unit

Each work unit is processed strictly according to a 17-step idempotent protocol:

1. **READ**: Inspect `docs/testing-roadmap.md` to identify the active unit.
