...
3. **Pedagogical Progression**: Structured across 12 progressive levels (Level 00 to Level 11), enabling a structured learning curve from core theoretical concepts to enterprise distributed architectures and exhaustive public API verification.
...
| **02** | Full Configuration | [`Level02_FullConfiguration.cs`](../samples/EricksonLopez.Concurrency.Showcase/Levels/Level02_FullConfiguration.cs) | All 7 `ConcurrencyOptions` properties configured: `DefaultResolutionStrategy`, `EnableDiagnostics`, `DefaultConflictClassification`, `RecordDetailedActivityTags`, `ThrowOnUnresolvedConflict`, `DefaultMaxAcquisitionTimeout`, `DefaultMaxExecutionTimeout`, `StripeCount`; custom resolver registration; full database dialect registrations. |
...
| **08** | Customization & Extensibility | [`Level08_CustomizationAndExtensibility.cs`](../samples/EricksonLopez.Concurrency.Showcase/Levels/Level08_CustomizationAndExtensibility.cs) | Implementing `IConcurrencyConflictResolver<T>`, domain-specific reconciliation with `ConflictResolution.Merged`, `DelegateConflictResolver`, explicit Last-Write-Wins (`LastWriteWinsConflictResolver`), `RefreshAndRetryConflictResolver` with default retry count, `MaxRetries`/`RetryDelayProvider` property access, and the `retryDelayProvider` exponential-backoff constructor overload. |
| **09** | Specialized Tokens & Locking | [`Level09_SpecializedTokensAndLocking.cs`](../samples/EricksonLopez.Concurrency.Showcase/Levels/Level09_SpecializedTokensAndLocking.cs) | Native database tokens: `XminConcurrencyToken` (PostgreSQL), `SqlServerRowVersionToken` (SQL Server), `OracleRowScnToken` (Oracle); `ConcurrencyVersion.Next()` (untyped overload); and all 5 PostgreSQL lock modes (`ForUpdate`, `ForUpdateNowait`, `ForUpdateSkipLocked`, `ForShare`, `ForNoKeyUpdate`) plus SQL Server, MySQL, MariaDB, and Oracle locking helpers. |
| **10** | Enterprise Architecture | [`Level10_EnterpriseArchitecture.cs`](../samples/EricksonLopez.Concurrency.Showcase/Levels/Level10_EnterpriseArchitecture.cs) | Clean Architecture + CQRS with `EricksonLopez.Mediator`, `ConcurrencyBehavior`, multi-tenancy SQL isolation, test double harness (`FakeConcurrencyController`, `ConcurrencyConflictBuilder`), queue-based `WithSuccessOnNextWrite(nextVersion)` and `WithConflictOnNextWrite(type, entityId, entityType, classification)` overloads, ASP.NET Core RFC 7807 `ConcurrencyProblemDetails`, and ADR-001 boundary separation. |
| **11** | Comprehensive Public API Verification | [`Level11_ComprehensiveApiCoverageDemo.cs`](../samples/EricksonLopez.Concurrency.Showcase/Levels/Level11_ComprehensiveApiCoverageDemo.cs) | Exhaustive executable verification across **16 sections**: `CasResult`, `ConcurrencyConflict` factories (including `ConcurrencyConflictType.AlreadyExists`), fluent builder, `ConflictResolution`, `ConcurrencyDiagnostics`, database classifiers, dialect SQL, `SqlServerRowVersionToken`, AspNetCore extensions, `FakeConcurrencyController`, domain contracts (with correct `IConcurrencyAware.ConcurrencyToken?` nullability), MySQL/MariaDB boolean predicates, `ConcurrencyVersion<TEntity>.Next()` typed overload + bridge constructor, `ConcurrencyConflict.Deleted()` entity-removal narrative (NonRetryable), `ConcurrencyOptions.ThrowOnUnresolvedConflict = true` automatic exception throw path, and **[16] Metadata propagation pipeline**: `ConcurrencyConflictBuilder.WithMetadata()` → `ConcurrencyErrors.FromConflict()` → `Error.Metadata` → `ConcurrencyProblemDetails.Extensions`. |
